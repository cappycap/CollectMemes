...
if (isset($_POST['userId']) and isset($_POST['memeId'])) {
  ...
  $achievement = array();
	...
	if ($check) {
	  ...
	  $queryUserInfo = "SELECT " . $rarity . "Count,spinsLeft,collectionSize,collectionSum,isPro,nextSpin FROM users WHERE id=?";
	  ...
	  if ($stmtUser = $con->prepare($queryUserInfo)) {
	    ...
	    $stmtUser->bind_result($rarityCount,$spinsLeft,$collectionSize,$collectionSum,$isPro,$nextSpin);
	    ...
	    if ($stmtUser->fetch()) {
	      ...
	      $stmtUser->close();
		      ...
		      $queryUpdateUser = "UPDATE users SET " . $rarity . "Count=" . $rarity . "Count+1, avgRank=?, collectionSize=?, collectionSum=?, lastCollect=?, spinsLeft=0 WHERE id=?";

		      $response['collectionSize'] = $newCollectionSize;

		      if ($stmtUpdateUser = $con->prepare($queryUpdateUser)) {

		        $stmtUpdateUser->bind_param("iiiiii",$newUserAvgRank,$newCollectionSize,$newCollectionSum,$time,$userId);
