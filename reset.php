...
<body>
  <div class="container">
    ...
    <nav class="navbar navbar-inverse navbar-expand-lg" role="navigation" style="margin-top:100px;">
      <a class="navbar-brand" href="#" style="padding-left:20px;">Reset Password</a>
    ...
    if (isset($_GET['v'])) {
      ...
      $receipt = $con->real_escape_string($_GET['v']);

      $q = "SELECT userId,time FROM resetRequests WHERE resetCode=?";
      ...
      if ($stmt = $con->prepare($q)) {
        ...
        if ($stmt->fetch()) {
              ...
              if ($valid) {
                ...
                if ($uStmt = $con->prepare($uQ)) {
                  ...
                  if ($uStmt->execute()) {
                    ...
                    $dQ = "DELETE FROM resetRequests WHERE resetCode=?";
          ...
          echo "<p style='text-align:center;'>Invalid reset link.</p>";
