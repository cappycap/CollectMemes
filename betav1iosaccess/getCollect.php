...
function spinMessage($spinsLeft, $isCountdown, $scheme, $nextSpin, $isLast) {
  ...
  $m['title'] = ($isLast) ? "Out of spins!" : "Spin to refresh!";

  if ($isCountdown) {

    $m['isHTML'] = 1;

    $m['title'] = "<html><head><style>body{ font-family:'HelveticaNeue-Light', Helvetica, Arial, sans-serif; background-color:" . $background . "; text-align:left; margin:0; padding:10px 0; color:" . $color . "; text-align:center; font-size:15px; }</style></head><body id='demo'></body>
    ...
    var x = setInterval(function() {
      ...
      if (distance < 0) {
        clearInterval(x);
        document.getElementById('demo').innerHTML = '" . $m['title'] . "';
      }
    }, 1000);
    </script>
    ...
    $m['title'] = $spinsLeftText;

    $m['color'] = ($spinsLeft == 0) ? "#e74c3c" : "#22a258";
        ...
        if ($s->fetch()) {
          ...
          if ($nextSpin < $time) {
            ...
            if ($spinsLeft == 11 or $spinsLeft == 0) {
              ...
              $cur['collectStatus'] = 1;
              $cur['spinMessage'] = spinMessage($newSpinsLeft, 0, $scheme, $nextSpin, 1);
