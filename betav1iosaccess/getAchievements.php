...
function getProgress($achievementId, $stage, $userId, $con, $width) {
  ...
  $p = ($max != 0) ? round((($min / $max) * 100),2) : 100;



  $progress['width'] = $width;

  $s = "<html><head><style>body { margin:0; height:24px; width:" . $width . "px; background:#dedede; } .bar { width:" . $p . "%; height:24px;background:#3498db; } </style></head><body><div class='bar'></div></body></html>";
...
if (isset($_POST['userId']) and isset($_POST['scheme']) and isset($_POST['width'])) {

  $width = intval(0.69 * intval($_POST['width']));
  ...
  foreach ($prog as $stage) {
    ...
    if ($s = $con->prepare($q)) {
      ...
      if ($s->fetch()) {
        ...
        $progInfo = getProgress($id, $stage, $u, $con, $width);

        $ach['bar'] = $progInfo['bar'];
        $ach['width'] = $progInfo['width'];
